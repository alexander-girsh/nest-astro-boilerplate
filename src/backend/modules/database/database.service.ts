...
export class DBService {
	...
	private createPool() {
		...
		this.pool.on('error', (err) => {
			this.logger.error(
				{ err },
				'Unexpected error of database connection',
			);
	...
	private onClientConnected(client: PG.PoolClient) {
		...
		client.on('error', (err: Error) => {
			this.logger.error(
				'DB client disconnected, trying to reconnect...',
				{
					err,
				},
			);
			...
			for (
				...
				try {
					...
					for await (const callback of client.afterCommitCallbacks) {
						...
						this.logger.debug(
							'after commit callback returned result',
							{
								callback: callback.toString(),
								callbackResult,
							},
			...
			if (this.config.PG_POOL_QUERIES_VERBOSE_MODE) {
				process.nextTick(() => {
					this.logger.info({
						...
						time_spent_ms:
							query_end_timestamp - query_start_timestamp,
						pool_state: {
